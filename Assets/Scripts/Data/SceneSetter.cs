    ...
    public class SceneSetter
    {
        ...
        public void GetSavedSceneState()
        {
            ...
            for (var i = 0; i < _savedSceneState.SaveObjectsList.Count; i++)
            {
                ...
                if (_listOfObjects[indexInList].GetComponent<BaseContainer>() is not null)
                {
                    _listOfObjects[indexInList].GetComponent<BaseContainer>().UpdateSubstance(_savedSceneState.SaveObjectsList[i].Substance);
                }
            }
        }
        ...
        private List<SaveObject> GetSaveContentListFromGameObjectList(List<GameObject> list)
        {
            ...
            for(var i = 0; i < _listOfObjects.Count; i++)
            {
                ...
                {
                    saveContent.Substance = _listOfObjects[i].GetComponent<BaseContainer>().Substance;
                }
                res.Add(saveContent);
            }
